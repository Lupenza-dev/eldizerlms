    ...
    <div class="page-content">
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-5">
            <div class="breadcrumb-title pe-3">
                <span class="text-lg font-bold text-slate-700">University Groups</span>
            </div>
            <div class="ps-3">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="bg-gradient-to-r from-slate-800 to-slate-900 px-6 py-4 flex items-center gap-3">
                ...
                <h6 class="text-sm font-semibold uppercase tracking-wider text-white mb-0">University Groups</h6>
            </div>
            <div class="p-6">
                    ...
                    <table id="example" class="table w-full" style="width:100%">
                        <thead>
                            <tr class="bg-slate-100 text-slate-600">
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider whitespace-nowrap">Link</th>
                        ...
                        @foreach ($groups as $group)
                            <tr class="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    @if($group->getFirstMediaUrl('images'))
                                        <img src="{{ $group->getFirstMediaUrl('images') }}" alt="Group Image" class="h-10 w-10 rounded-lg object-cover border-2 border-slate-200 shadow-sm">
                                    @else
                                        <span class="inline-flex items-center px-2 py-1 rounded-md bg-slate-100 text-slate-400 text-xs">No Image</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-sm text-slate-500 whitespace-nowrap">{{ date('d M Y', strtotime($group->created_at)) }}</td>
                                <td class="px-4 py-3 text-sm font-semibold text-slate-800 whitespace-nowrap">{{ $group->name }}</td>
                                <td class="px-4 py-3 text-sm text-blue-600 whitespace-nowrap">
                                    <a href="{{ $group->link }}" target="_blank" class="hover:underline truncate max-w-xs block">{{ $group->link }}</a>
                                </td>
                                <td class="px-4 py-3 text-sm text-slate-600 whitespace-nowrap">{{ $group->college?->name }}</td>
                                <td class="px-4 py-3 whitespace-nowrap">{!! $group->status_formatted !!}</td>
                                <td class="px-4 py-3 whitespace-nowrap">
        ...
        <div class="modal-content border-0 shadow-xl rounded-2xl overflow-hidden">
            <div class="bg-gradient-to-r from-emerald-600 to-emerald-800 px-6 py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h5 class="text-white font-semibold text-base mb-0">Group Registration</h5>
                </div>
                ...
                <form action="" id="registration_form">
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" class="form-control rounded-lg text-sm" placeholder="Enter group name..." required>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Link</label>
                            <input type="text" name="link" class="form-control rounded-lg text-sm" placeholder="Enter group link..." required>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Image</label>
                    ...
                    <div class="flex justify-end gap-2 mt-5 pt-4 border-t border-slate-100">
                        ...
                        <button type="submit" class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium px-5 py-2 rounded-lg transition-colors" id="reg_btn">
        ...
        <div class="modal-content border-0 shadow-xl rounded-2xl overflow-hidden">
            <div class="bg-gradient-to-r from-cyan-600 to-cyan-800 px-6 py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h5 class="text-white font-semibold text-base mb-0">Edit Group</h5>
                </div>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-6">
